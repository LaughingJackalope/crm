...
class OutboxEventRepository {
    ...
    fun findPending(batchSize: Int = 100): List<OutboxEventEntity> =
        OutboxEventEntity.list("status = ?1 ORDER BY createdAt ASC", OutboxStatus.PENDING).take(batchSize)

    fun findFailedForRetry(maxRetries: Int = 5, retryThreshold: Instant = Instant.now().minusSeconds(60)): List<OutboxEventEntity> =
        OutboxEventEntity.list("status = ?1 AND retryCount < ?2 AND createdAt < ?3 ORDER BY createdAt ASC",
            OutboxStatus.FAILED, maxRetries, retryThreshold).take(100)
