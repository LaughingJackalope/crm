package com.crm.ciam.infrastructure.persistence

import jakarta.enterprise.context.ApplicationScoped
import java.time.Instant
import java.util.UUID

/**
 * Repository for the transactional outbox.
 *
 * Uses the Panache companion pattern (same as CustomerEntity) for
 * consistency with the existing codebase.
 */
@ApplicationScoped
class OutboxEventRepository {

    /**
     * Persist an outbox event within the current transaction.
     * Must be called from within an active JPA transaction.
     */
    fun save(event: OutboxEventEntity) {
        event.persist()
    }

    fun findPending(batchSize: Int = 100): List<OutboxEventEntity> =
        OutboxEventEntity.list("status = ?1 ORDER BY createdAt ASC", OutboxStatus.PENDING)
            .take(batchSize)

    fun findFailedForRetry(
        maxRetries: Int = 5,
        retryThreshold: Instant = Instant.now().minusSeconds(60),
    ): List<OutboxEventEntity> =
        OutboxEventEntity.list(
            "status = ?1 AND retryCount < ?2 AND createdAt < ?3 ORDER BY createdAt ASC",
            OutboxStatus.FAILED,
            maxRetries,
            retryThreshold,
        ).take(100)

    fun markFailed(eventId: UUID) {
        val entity = OutboxEventEntity.find("eventId", eventId).firstResult() ?: return
        entity.retryCount++
        entity.status = if (entity.retryCount >= 5) OutboxStatus.FAILED else OutboxStatus.PENDING
        entity.persist()
    }

    fun delete(eventId: UUID) {
        OutboxEventEntity.delete("eventId", eventId)
    }

    fun countPending(): Long =
        OutboxEventEntity.count("status", OutboxStatus.PENDING)

    fun countFailed(): Long =
        OutboxEventEntity.count("status", OutboxStatus.FAILED)

    fun remove(eventId: UUID) {
        OutboxEventEntity.delete("eventId", eventId)
    }
}
