-- Transactional Outbox and Idempotency Log for the Billing bounded context.
...
CREATE INDEX IF NOT EXISTS ix_outbox_event_pending
    ON billing.outbox_event (created_at ASC)
...
CREATE TABLE IF NOT EXISTS billing.incoming_event_log (
    event_id        UUID            NOT NULL PRIMARY KEY,
    event_type      VARCHAR(128)    NOT NULL,
    source          VARCHAR(32)     NOT NULL,
    entity_id       VARCHAR(36)     NOT NULL,
...
CREATE INDEX IF NOT EXISTS ix_incoming_event_log_type
    ON billing.incoming_event_log (event_type, source);
