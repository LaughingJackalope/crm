...
echo ""
echo "▶ Deploying infrastructure (Postgres × 6, Kafka, Zookeeper)"
...
kubectl -n "${NAMESPACE}" wait --for=condition=ready pod \
    --selector="app in (postgres-ciam, postgres-sales, postgres-billing, postgres-support, postgres-marketing, postgres-communication, kafka, zookeeper)" \
...
echo ""
echo "  Services:"
echo "    CIAM          → http://localhost:8081/ciam/contacts"
echo "    Sales         → http://localhost:8082/sales/opportunities"
echo "    Billing       → http://localhost:8083/billing/invoices"
echo "    Support       → http://localhost:8084/support/tickets"
echo "    Marketing     → http://localhost:8085/marketing/campaigns"
echo "    Communication → http://localhost:8086/communication/messages"
echo ""
echo "  Useful commands:"
echo "    kubectl -n ${NAMESPACE} get pods"
echo "    kubectl -n ${NAMESPACE} logs -f deployment/ciam-service"
echo "    kubectl -n ${NAMESPACE} port-forward svc/ciam-service 8081:8081"
echo ""
echo "  Teardown:"
